...
#ifndef PreliminaryBgSelector_h
#define PreliminaryBgSelector_h
...
class PreliminaryBgSelector : public TGRSISelector {
 ...
 public :
   ...
   PreliminaryBgSelector(TTree * /*tree*/ =0) : TGRSISelector(), fGrif(0), fScep(0), fZDS(0) {
      SetOutputPrefix("PreliminaryBg");
   ...
   virtual ~PreliminaryBgSelector() { }
   virtual Int_t   Version() const { return 2; }
   void CreateHistograms();
   void FillHistograms();
   void InitializeBranches(TTree *tree);

   ClassDef(PreliminaryBgSelector,2);
};

#endif

#ifdef PreliminaryBgSelector_cxx
void PreliminaryBgSelector::InitializeBranches(TTree* tree) {
   if (!tree) return;
   tree->SetBranchAddress("TGriffin", &fGrif);
   tree->SetBranchAddress("TZeroDegree", &fZDS);
}

#endif // #ifdef PreliminaryBgSelector_cxx
